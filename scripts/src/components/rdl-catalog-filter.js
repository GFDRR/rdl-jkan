import $ from "jquery";
import { chain, get, omit, defaults } from "lodash";
...
function prep_catalog(catalog, params, datasetsForCatalog) {
  ...
  const itemParams = selected
    ? omit(params, "catalog")
    : defaults({ catalog: catalogSlug }, params);
  return {
    title: catalog,
...
export default class {
  constructor(opts) {
    ...
    const catalogsMarkup = consolidated
      .sort((a, b) => {
  ...
  _catalogsWithCount(datasets, params) {
    return chain(datasets)
      .groupBy((dataset) =>  dataset.catalog ?? "Other")
      .flatMap(function (datasetsForCatalog, catalog) {
        var catalogs = catalog.split(",");
        var collated = catalogs.map((c) =>
          prep_catalog(c, params, datasetsForCatalog),
        );
