---
catalog: Humanitarian Data Exchange
contact_point:
  email: null
  id: attribution_contact
  name: WFP Advanced Disaster Analysis & Mapping
  url: https://data.humdata.org/dataset/c8d4ab35-e0b6-405b-9cc4-fe3e709a7132
creator:
  email: null
  id: attribution_creator
  name: WFP ADAM
  url: https://data.humdata.org/dataset/c8d4ab35-e0b6-405b-9cc4-fe3e709a7132
dataset_id: rdls_he-phl_wfpadam_cyclonetropicaldepression
description: 'ADAM ID: 1001032_4 Cyclone (tropical depression) during the period Nov
  12 2023-Nov 13 2023 in . It impacted 0 people. [Source: This metadata record was
  automatically extracted from the Humanitarian Data Exchange (HDX); Original dataset:
  https://data.humdata.org/dataset/c8d4ab35-e0b6-405b-9cc4-fe3e709a7132]'
details: 'Methodology: ADAM is an operational system for collecting, analysing and
  mapping geospatial and socio-economic information following sudden onset humanitarian
  emergencies. Currently operational for floods, earthquakes and tropical storms,
  ADAM issues alerts and response dashboards aggregating relevant, evidence-based,
  near-real time risk and impact information.'
exposure:
- category: population
  dimension: population
  quantity_kind: count
  taxonomy: null
extra_attributions: []
hazard:
  calculation_method: inferred
  disaster_identifiers: ''
  hazard_analysis_type: empirical
  hazard_type: convective_storm
  intensity: sws_3s:km/h
  occurrence_range: ''
  processes: tornado
  seasonality: ''
license: CC-BY-SA-4.0
loss: null
project: null
publisher:
  email: null
  id: attribution_publisher
  name: WFP Advanced Disaster Analysis & Mapping
  url: https://data.humdata.org/dataset/c8d4ab35-e0b6-405b-9cc4-fe3e709a7132
purpose: null
resources:
- coordinate_system: null
  description: Shape File
  download_url: https://data.humdata.org/dataset/c8d4ab35-e0b6-405b-9cc4-fe3e709a7132/resource/45f62a1a-5c15-4c6f-b88a-ae11b8f26dbf/download/1001032-6-adam-ts-1001032-6-shp.zip
  format: Shapefile (shp)
  id: hdx_res_45f62a1a
  media_type: null
  spatial_resolution: null
  temporal:
    duration: P1D
    end: '2023-11-13'
    start: '2023-11-12'
    temporal_resolution: null
  title: 1001032-6-adam-ts-1001032-6-shp.zip
risk_data_type:
- hazard
- exposure
schema: rdl-03
slug: rdls_he-phl_wfpadam_cyclonetropicaldepression
spatial:
  countries:
  - PHL
  scale: national
title: 'Philippines: Cyclone - Tropical depression - Nov 2023'
version: null
vulnerability: null
---
